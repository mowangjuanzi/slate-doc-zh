...
1. **所有的 `Element` 节点都必须包含至少一个 `Text` 子节点** —— 甚至是 [空元素](./02-nodes.md#voids)。如果元素节点未包含任何子节点，则会添加一个空文本节点作为其唯一的子节点。存在该约束以确保选区的锚点和焦点（依赖引用文本节点）始终可以放置在任何节点内。这样，空元素（或者 void 元素）将不可选。
